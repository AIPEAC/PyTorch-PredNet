...
class PredNet(nn.Module):
	def __init__(self, input_size, R_channels, A_channels, p_max=1.0, output_mode='error',
		...
		if self.use_transformer:
			...
			for l in range(self.n_layers):
				# Independent alpha for E, R, Ahat at each layer
	...
	def step(self, a, states):
		...
		for l in range(self.n_layers):
			...
			# #TODO: PredNet Transformer - Apply transformer and fusion for each layer
			if self.use_transformer:
				transformer_E = getattr(self, 'transformer_E{}'.format(l))
				transformer_R = getattr(self, 'transformer_R{}'.format(l))
				transformer_Ahat = getattr(self, 'transformer_Ahat{}'.format(l))

				# Apply transformers to each component
				...
				ahat_transformed = self._apply_transformer_to_feature_map(a_hat, transformer_Ahat)

				# #TODO: PredNet Transformer - Fuse transformer outputs with original outputs using independent learnable weights per layer and component
				alpha_e = torch.sigmoid(getattr(self, 'alpha_E{}'.format(l)))
				alpha_r = torch.sigmoid(getattr(self, 'alpha_R{}'.format(l)))
				alpha_ahat = torch.sigmoid(getattr(self, 'alpha_Ahat{}'.format(l)))
				
				e = alpha_e * e_transformed + (1 - alpha_e) * e
				R_layers[l] = alpha_r * r_transformed + (1 - alpha_r) * R_layers[l]
				a_hat = alpha_ahat * ahat_transformed + (1 - alpha_ahat) * a_hat
