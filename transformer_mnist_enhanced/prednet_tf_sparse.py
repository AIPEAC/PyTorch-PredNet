...
class PredNet(nn.Module):
	def __init__(self, input_size, R_channels, A_channels, p_max=1.0, output_mode='error',
		...
		if self.use_transformer:
			...
			current_h, current_w = self.input_size # Layer 0 size is input size (64, 64)
			
			e_heads = self._get_valid_num_heads(e_dim, num_transformer_heads)
			
			# #TODO: Transformation Enhanced - Pass height/width for Positional Encoding
			e_transformer = TransformerBlock(e_dim, height=current_h, width=current_w, num_heads=e_heads, spatial_bias=True)
			setattr(self, 'transformer_E0', e_transformer)

			# #TODO: PredNet Transformer Sparse - Learnable fusion weight alpha for Layer 0 E
			setattr(self, 'alpha_E0', nn.Parameter(torch.tensor(0.45)))
