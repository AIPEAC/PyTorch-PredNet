...
# Image processing function from process_kitti.py
...
with Image.open(f"img/{reading_files}") as img:
	...
	img = np.asarray(img)
	print(f"img original shape {img.shape}")
	# Process image using process_kitti function
	img = process_im(img, desired_im_sz)
	print(f"img processed shape {img.shape}")
...
# Load model with CPU mapping to handle models saved with CUDA
